...
def max_pool_with_argmax(net, stride):
	#使用mask保存每个最大值的位置，只支持GPU
	_, mask = tf.nn.max_pool_with_argmax(value, ksize = [1, stride, stride, 1], strides = [1, stride, stride, 1], padding = 'SAME')
	#将反向传播的mask梯度计算停止
	mask = tf.stop_gradient(mask)
	#计算最大池化操作
	net = tf.nn.max_pool(value, ksize=[1, stride, stride, 1],strides=[1, stride, stride, 1], padding='SAME') 
	#将池化结果和mask返回
	return net,mask
...
def inputs(data_dir, name = 'input'):
	with tf.name_scope(name):
		filequeue = tf.train.string_input_producer([data_dir])
		image = read_and_decode(filequeue)
		images = tf.train.shuffle_batch([image], batch_size = BATCH_SIZE, num_threads = 8, capacity = 20000 + 3 * BATCH_SIZE, min_after_dequeue = 20000)
		return images
